...
Read `.orbit/plans/scholar/tasks.json`. Find the next incomplete topic (first where `done` is `false`).

Break this topic into atomic research tasks and write them to `.orbit/plans/scholar/atomic/current.json`:
